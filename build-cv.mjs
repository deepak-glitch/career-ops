...
const competenciesArg = (args[2] || '').split('|');
const projectsOrderArg = args[3]; // e.g. "claims,manga,dream,yolo"
const outHtml = args[4] || `/tmp/cv-${slug}.html`;
...
const COMPETENCIES = (competenciesArg.filter(Boolean).length ? competenciesArg : COMPETENCIES_DEFAULT);
...
const PROJECT_DEFS = {
  ...
  manga: `
    <div class="project">
      ...
      <div class="project-desc">Real-time manga translation extension shipped to the Chrome Web Store. Multi-provider abstraction (4 AI vision providers: Claude Sonnet, GPT-4o mini, GPT-4.1 Nano, Ollama) with per-provider payload handling for worst-case failure isolation. 29 sites supported; 7-day cache; privacy policy and narrowed permissions.</div>
      <div class="project-tech">TypeScript, Manifest V3, Service Workers, OpenAI / Anthropic / Ollama</div>
    </div>`,
  dream: `
    <div class="project">
      <span class="project-title">Dream Decoder - Multimodal Creative Platform</span>
      <div class="project-desc">End-to-end multimodal pipeline: text description, intermediate prompt-transformation layer, grounded image generation. ~30% contextual alignment gain, ~25-30% first-pass image success.</div>
  ...
  pixel: `
    <div class="project">
      ...
      <div class="project-desc">Stable Diffusion fine-tuning + LoRA for identity-consistent pixel character synthesis with sprite-sheet export. ControlNet + OpenPose/MediaPipe for pose conditioning; latent-space consistency for animation smoothness; LLM-based agent orchestrator decomposing high-level prompts into generation tasks.</div>
      <div class="project-tech">PyTorch, Hugging Face Diffusers, Stable Diffusion + LoRA, ControlNet, FastAPI, Docker</div>
    </div>`,
  efarming: `
    <div class="project">
      <span class="project-title">E-Farming Digital Marketplace (Founder &amp; Full-Stack)</span>
      <div class="project-desc">Independently designed and shipped PHP/MySQL/Bootstrap AgriTech marketplace connecting small-scale farmers with buyers; 80-120 active users in phase 1; cart, reviews, community blogging features for repeat engagement.</div>
      <div class="project-tech">PHP, MySQL, Bootstrap</div>
    </div>`,
};

const order = (projectsOrderArg || 'claims,manga,dream,yolo').split(',').map(s => s.trim()).filter(Boolean);
const PROJECTS = order.map(k => PROJECT_DEFS[k] || '').filter(Boolean).join('');
