...
<!-- Stories added from 2026-04-22 batch 7D -->

### [GenAI Training-Data Ops] Evaluation Pipelines as the Sales Artifact, Not the Internal Metric
**Source:** Report #104 Appen (Forward Deployed Engineer, Applied AI & Research)
**S (Situation):** Progress Solutions was shipping a healthcare RAG system where the retrieval output was going to clinical stakeholders who had to sign off before deployment. A raw accuracy number wouldn't clear procurement — stakeholders needed evidence they could audit.
**T (Task):** Build an evaluation pipeline whose outputs were legible to non-ML stakeholders and could stand up as an audit artifact, not just an internal model-selection tool.
**A (Action):** Structured evaluation harness over a labeled clinical knowledge corpus. Measured retrieval-precision uplift, hallucination rate, and retrieval-grounded response alignment as first-class outputs. Wrote each eval run to a human-readable report rather than a Jupyter notebook. Paired every metric with an example set so clinicians could see where the system was confident, where it hedged, and where it failed, rather than trusting a single scalar.
**R (Result):** ~35% retrieval precision gain, >30% hallucination reduction, >90% retrieval-grounded response alignment. Clinical ops signed off on the basis of the eval report; the evaluation pipeline became the hand-off artifact in subsequent deployments.
**Reflection:** For data-quality and evaluation-centric FDE roles, the evaluation pipeline is a deliverable to the customer, not an engineering tool for the modeler. Design it from the outside in: what does the reviewer need to see to trust the system? Then back into the metrics and logging.
**Best for questions about:** Evaluation pipelines, data-quality ownership, GenAI training data, FDE delivery, hallucination detection, audit artifacts, stakeholder sign-off, translating business intent into technical workflows.

### [Compliance-Sensitive Delivery] RBAC + Audit Logging as a Cross-Industry FDE Primitive
**Source:** Report #105 Symmetry Systems (DSPM FDE L4); generalizable to compliance-adjacent FDE contexts
**S (Situation):** At Emerson, the Synthesis Order-to-Cash ERP served financial modules (contracts, nominations, allocations, invoicing) in a compliance-sensitive oil-gas environment. Audit logging and RBAC were sign-off criteria, not nice-to-haves.
**T (Task):** Support RBAC implementation + audit logging for financial modules while keeping release cadence on a Jenkins-driven CI/CD pipeline with structured release validation + rollback checkpoints.
**A (Action):** Schema and stored-procedure deployments gated behind Jenkins pipelines with version-controlled SQL scripts; deployment errors instrumented and traced back to rollback points. RBAC implementation for financial modules paired with SQL Server DMV + Grafana dashboards for long-running queries, deadlocks, and CPU/I/O contention — so compliance officers could see, not just be told, that the system was under control.
**R (Result):** Deployment errors down >30%; release cycle time down ~35-40%; incident recurrence down ~25%; root-cause resolution speed up ~30%. Compliance posture held across the ERP.
**Reflection:** Compliance work trains you to treat logs, role definitions, and rollback as product features, not ops overhead. The specifics change when you cross industries (AWS IAM instead of SQL RBAC; DSPM audit events instead of T-SQL DMVs) but the primitive is the same — make the control surface observable.
**Best for questions about:** FDE into security / compliance-sensitive customers, RBAC design, audit logging, CI/CD for compliance environments, Jenkins + rollback discipline, translating regulated-vertical experience across industries.

### [Multi-Provider Agent Orchestration] Manga Lens as Proof of Production Multi-Model Routing
**Source:** Reports #106 ELSA (Production AI Agents) + #101 Rockstar (Applied AI for PE); any role that asks "how do you handle multi-model cost / latency / quality tradeoffs"
**S (Situation):** Building a Chrome extension that translated manga / webtoon panels in real time across 29 different sites meant choosing between four different AI vision providers (Claude Sonnet, GPT-4o mini, GPT-4.1 Nano, Ollama/minicpm-v local). Each provider had its own failure modes, payload formats, and cost/latency profile.
**T (Task):** Ship a production extension on the Chrome Web Store that handled four vision providers gracefully — with isolated failures, per-provider payload differences, and a caching layer that made the cost model viable.
**A (Action):** Manifest V3 extension with content scripts, service workers, and captureVisibleTab for panel capture. Multi-section capture pipeline for tall webtoon panels using viewport-slice screenshots with coordinate remapping and dedup. Per-provider payload handling (WebP for cloud, JPEG for Ollama local to work around a CUDA crash). Seven-day per-panel translation cache. Per-domain selector configs so new sites could be onboarded without a code release. Narrowed host permissions + privacy policy before Chrome Web Store review.
**R (Result):** Shipped on the Chrome Web Store. Four-provider orchestration held across 29 sites with provider-specific failure isolation (a CUDA crash on the local provider didn't take out the extension). The cache was the single biggest cost win — more so than picking a "cheaper" model.
**Reflection:** Multi-provider orchestration is usually framed as a cost problem; it's actually a failure-isolation problem. Provider-specific payload handling, per-provider timeouts, and aggressive caching are the levers. Production robustness comes from assuming any single provider will fail on any given request.
**Best for questions about:** Multi-model orchestration, cost/latency/quality tradeoffs, provider failure isolation, Chrome extensions + Manifest V3, agent-level routing, production AI features shipped to real users.

### [Enterprise Integration Ownership] Healthcare RAG Rollout as Partner-Grade Delivery
**Source:** Reports #078 Hopper HTS Assist + #079 Tavily FDE Partnerships + #080 Pareto.AI FDE SWE; any FDE role that asks "how do you own end-to-end delivery to a non-technical stakeholder?"
**S (Situation):** Clinical ops teams at Progress Solutions needed RAG-grounded answers over EHR extracts, appointment histories, and policy documents. They had limited tolerance for AI hallucinations and a HIPAA-governed data boundary. The system had to be trustworthy enough to inform care-workflow decisions.
**T (Task):** Ship a healthcare RAG system that met the stakeholders' governance bar, delivered measurable retrieval quality, and could be maintained + audited by non-AI staff after launch.
**A (Action):** Recursive semantic chunking + transformer embeddings for retrieval; retrieval-grounded response alignment with a measurable eval pipeline; de-identification before embedding; data-lineage documentation; evaluation audit trails; stakeholder-facing system-limitation docs ("when this system is wrong") so clinical users knew the boundaries.
**R (Result):** ~35% contextual retrieval precision gain; >30% irrelevant-retrieval reduction; >90% grounded response alignment in eval; stakeholder sign-off accelerated because the limitation docs made the governance legible. Downstream model instability dropped ~40% once preprocessing reliability hit 98%.
**Reflection:** The CV metrics are real but the load-bearing work was the limitation docs. Enterprise / partner delivery is 50% writing — making the failure modes legible is what earns the right to ship. FDE roles are graded on this, not just the model quality.
**Best for questions about:** FDE + partner delivery, enterprise stakeholder management, regulated-domain AI, HIPAA / audit-trail work, making AI systems legible to non-technical users, RAG evaluation discipline, stakeholder-facing documentation.

### [Agentic Schema Contracts] Multi-Agent Claims Fraud Pipeline
**Source:** Reports #077 Variance FDE + #078 Hopper HTS + #079 Tavily; any role that asks "how do you stop cascading hallucinations in multi-agent systems?"
**S (Situation):** A multi-agent healthcare-claims processing pipeline had five sequential agents (Intake Normalization → Validation → Consistency Analysis → Duplicate Detection → Fraud Risk Scoring). Early iterations had cascading hallucinations: one agent's bad output poisoned everything downstream, and the end-to-end accuracy collapsed.
**T (Task):** Stop cascading hallucinations without rewriting the whole pipeline in a monolithic prompt. Keep each agent independently testable + replaceable.
**A (Action):** Schema-validated JSON contracts between every agent hop (structured outputs enforced at the boundary, not the prompt). RAG-grounded CPT/ICD validation against vector-indexed policy documents. Approximate-nearest-neighbor similarity search for duplicate claim detection. Explainable risk scoring with audit-ready reasoning traces. Each agent could be swapped independently because the contract made the seam clean.
**R (Result):** Zero cascade failures in eval. Audit-ready reasoning traces enabled QA review and regulatory explainability. The pipeline became a template — future agent-based projects reused the schema-contract pattern.
**Reflection:** Agent orchestration gets better when you constrain what each agent can emit, not when you give each agent more freedom. Schema contracts at agent boundaries beat "bigger prompts" — they make the system testable, replaceable, and explainable. This is also the load-bearing design for partner-facing AI where "why did the system say that?" is a question you answer weekly.
**Best for questions about:** Multi-agent pipelines, agent reliability, cascading hallucinations, structured outputs, schema validation for AI, explainable AI, audit trails, regulated-domain agentic AI, FDE architectural decisions.

### [Full-Stack AI Product Delivery] Dream Decoder Multimodal Platform
**Source:** Report #080 Pareto.AI; any role that asks "do you actually ship full-stack code, or just models?"
**S (Situation):** Deepak wanted to prove he could ship a multimodal AI product end-to-end — not just the model layer, but the full user-facing app with coordinated provider APIs.
**T (Task):** Build a consumer-facing app that interprets dreams, generates poetic reinterpretations, and synthesizes dreamscapes via coordinated multimodal APIs.
**A (Action):** Full-stack FastAPI + React + TypeScript + Vite + Tailwind. Coordinated Perplexity Sonar for interpretation + Replicate image models for 16:9 visuals. Introduced intermediate structured prompt-transformation layers between raw user input and the provider APIs — a small but load-bearing architectural decision.
**R (Result):** ~30% contextual alignment improvement and ~25-30% first-pass image success-rate gain versus a naive direct-prompt orchestration baseline. Shipped as a working full-stack app.
**Reflection:** The structured prompt-transformation layer is the pattern — a thin seam between raw user input and provider APIs buys alignment + success rate without retraining anything. It's also the cleanest place to add logging, caching, and eval hooks later. I use this pattern as a default now.
**Best for questions about:** Full-stack AI delivery, multimodal orchestration, React + FastAPI + TypeScript chops, prompt engineering as an architectural discipline, lab-customer technical communication (Pareto.AI asked this framing).
