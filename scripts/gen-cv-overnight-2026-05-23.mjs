#!/usr/bin/env node
// scripts/gen-cv-overnight-2026-05-23.mjs <slug> <date> <region:US|Intl> <summary-override>
import { readFileSync, writeFileSync, mkdirSync } from 'fs';
import { execSync } from 'child_process';
import yaml from 'js-yaml';
import path from 'path';

const [slug, date, region, summaryOverride] = process.argv.slice(2);
if (!slug || !date || !region) {
  console.error('usage: gen-cv-overnight.mjs <slug> <date> <US|Intl> [summary-override]');
  process.exit(1);
}

const ROOT = process.cwd();
const TEMPLATE = readFileSync(path.join(ROOT, 'templates/cv-template.html'), 'utf-8');
let profile = {};
try {
  profile = (yaml.load(readFileSync(path.join(ROOT, 'config/profile.yml'), 'utf-8')) || {}).candidate || {};
} catch (e) {
  profile = {};
}
...
const summary = (summaryOverride && summaryOverride.length > 30) ? summaryOverride : BASE_SUMMARY;
...
const placeholders = {
  ...
  '{{PHONE}}': profile.phone || '(330) 554-0770',
  '{{EMAIL}}': profile.email || 'deepakmallampati00@gmail.com',
  '{{LINKEDIN_URL}}': profile.linkedin ? (profile.linkedin.startsWith('http') ? profile.linkedin : `https://${profile.linkedin}`) : 'https://www.linkedin.com/in/deepak-mallampati/',
  '{{LINKEDIN_DISPLAY}}': (profile.linkedin || 'linkedin.com/in/deepak-mallampati').replace(/^https?:\/\//, ''),
...
const outDir = region === 'Intl' ? `output-intl/${date}` : `output/${date}`;
const out = `${outDir}/cv-deepak-mallampati-${slug}-${date}.pdf`;
