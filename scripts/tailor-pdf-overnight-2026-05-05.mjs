#!/usr/bin/env node
// Overnight 2026-05-05 batch — Fieldguide AI Quality + Cresta AI Language Engineer.
...
const ROLES = {
  'fieldguide-ai-quality': {
    summary: `${BASE} Targeting Fieldguide's AI Engineer (Quality) seat — multi-agent claims pipeline w/ schema-validated JSON contracts (5 agents, cascading-error containment), RAG-grounded CPT/ICD validation (~35% precision, >90% groundedness), Manga Lens 4-provider production comparison harness (effectiveness/consistency/latency/cost), agentic LLM workflows w/ tool discipline + grounding rules (~25% stability gain), and structured logging + load simulation (~30% defect reduction) translate 1:1 to Fieldguide's eval infrastructure + LangSmith/LangGraph + observability + production feedback loops for vertical-AI audit + assurance. Open to SF or Remote-USA.`,
    comps: ['AI Engineer Quality', 'Evaluation Infrastructure', 'LangSmith / LangGraph', 'Multi-Agent Pipelines', 'Schema-Validated JSON Contracts', 'RAG / Grounding / Eval Harness', 'Multi-Provider Comparison', 'Observability / Tracing', 'Python / TypeScript / React / Postgres', 'SF or Remote-USA'],
  },
  'cresta-ai-language-engineer': {
    summary: `${BASE} Targeting Cresta's AI Language Engineer seat — Suvidha transformer-based hierarchical video summarization (5,000+ sessions, 60-70% review-time reduction, ~85% human-curation alignment), healthcare RAG w/ recursive semantic chunking + transformer embeddings (~35% precision, >90% groundedness), agentic LLM workflows w/ tool discipline + grounding rules (>30% hallucination reduction, ~25% stability gain), Manga Lens 4-provider AI vision production routing (TypeScript Manifest V3 + per-provider payload abstraction), and Dream Decoder structured prompt transformation (~30% contextual alignment gain) translate 1:1 to Cresta's LLM workflows + NLP components (intent / entity / summarization / RAG) + ASR/TTS-adjacent eval frameworks for conversational-AI contact-center stack. Remote-US.`,
    comps: ['AI Language Engineer', 'NLP / RAG / Summarization', 'LLM Workflows', 'Prompt Design + Eval Frameworks', 'Conversational AI / Agent Assist', 'Hierarchical Summarization', 'Multi-Provider AI Integration', 'Hugging Face / PyTorch / Python', 'FastAPI + Docker', 'Remote-US'],
  },
};
...
if (targets.length === 0) {
  console.log('Usage: node tailor-pdf-overnight-2026-05-05.mjs <slug>...');
