#!/usr/bin/env node
// Overnight 2026-05-05 batch 7 — P-1 AI FDE + Vettura AI/ML + Littlebird Applied AI.
...
const ROLES = {
  'p1-ai-fde': {
    summary: `${BASE} Targeting P-1 AI's Forward Deployed Engineer seat — Manga Lens 4-provider AI vision Chrome MV3 extension w/ provider-specific payload abstraction (consumer production), Agentic Healthcare Claims 5-stage pipeline w/ schema-validated JSON contracts (cascading-error containment), Progress Solutions HIPAA-conscious RAG + agentic LLM workflows + FastAPI/Docker structured-logging deployments, and Dream Decoder multi-API orchestration translate 1:1 to P-1 AI's Archie customer-deployment lifecycle (pre-sales scoping → integrations → fine-tuning → production support). HIPAA discipline ports to industrial safety-critical adjacency. US/Canada Remote w/ quarterly San Mateo travel.`,
    comps: ['Forward Deployed Engineer', 'Customer Technical Lead', 'Production AI Systems', 'Multi-Agent Pipelines', 'Schema-Validated JSON Contracts', 'RAG / Grounding / Eval', 'Multi-Provider AI Integration', 'Python / FastAPI / Docker', 'HIPAA-Conscious / Regulated Domain', 'US/Canada Remote'],
  },
  'vettura-aiml': {
    summary: `${BASE} Targeting Vettura's AI/ML Engineer seat — breadth across NLP (Suvidha transformer-based hierarchical video summarization, Progress agentic LLM workflows), Computer Vision (YOLOv8 Driver Drowsiness ~30% latency cut + ~25% blink false-positive reduction, Manga Lens 4-provider AI vision Chrome extension), and Generative AI (Dream Decoder multimodal orchestration with structured intermediate prompts ~30% alignment gain, Agentic Pixel Character Synthesis Stable Diffusion + LoRA + ControlNet) translates 1:1 to Vettura's cross-domain ML team. End-to-end pipelines via FastAPI + Docker + structured logging deliver ~30% post-deploy defect reduction. Remote.`,
    comps: ['AI/ML Engineer', 'NLP / RAG / Summarization', 'Computer Vision / YOLOv8', 'Generative AI / Diffusion / LoRA', 'End-to-End ML Pipelines', 'Python / PyTorch / Hugging Face', 'FastAPI + Docker', 'Multi-Provider AI Integration', 'Cloud-Ready Deployment', 'Remote'],
  },
  'littlebird-applied-ai': {
    summary: `${BASE} Targeting Littlebird's Applied AI Engineer (Agent Intelligence & Retrieval) seat — Progress Solutions healthcare RAG w/ recursive semantic chunking + transformer embeddings (~35% retrieval precision, >90% grounded alignment, ~25% agent stability), Suvidha document Q&A w/ semantic chunking + embedding retrieval (~30% hallucination reduction, >85% contextual accuracy), Agentic Healthcare Claims schema-validated JSON contracts between agents (cascading-error containment), Manga Lens 4-provider AI vision Chrome MV3 (TypeScript production OOP), and Dream Decoder structured intermediate prompt transformation (~30% alignment gain — functionally a latent-space re-rank) translate 1:1 to Littlebird's hybrid search pipeline (BM25 + vector + RRF + re-ranker) and agent logic for personal-AI thought-partner. Remote-USA.`,
    comps: ['Applied AI Engineer', 'Agent Intelligence', 'Retrieval / RAG', 'Hybrid Search (BM25 + Vector + RRF)', 'Re-Ranking / Embeddings', 'Schema-Validated Agent Contracts', 'Python / TypeScript', 'Vector Databases', 'Personal AI / Consumer ML', 'Remote-USA'],
  },
};
...
if (targets.length === 0) {
  console.log('Usage: node tailor-pdf-overnight-2026-05-05-batch7.mjs <slug>...');
