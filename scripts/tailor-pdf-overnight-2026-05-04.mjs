#!/usr/bin/env node
// Overnight 2026-05-04 batch — WeRide CV New Grad + Arize AI Instrumentation + Labelbox Full-Stack AI Data Platform.
...
const ROLES = {
  'weride-newgrad-cv': {
    summary: `${BASE} Targeting WeRide's New Grads 2026 - Software Engineer, Computer Vision (Perception team) seat — real-time YOLOv8 detection+classification (~30% inference latency reduction), sliding-window confidence aggregation (~25% false-positive reduction), Stable Diffusion + ControlNet + OpenPose multi-modal pose-controlled CV pipeline, multi-provider AI vision Chrome Web Store extension (Manga Lens, 4 vision providers, 29 sites), and transformer-based video summarization (5,000+ sessions, 60-70% review-time reduction) translate 1:1 to WeRide's perception + scene understanding stack. Open to San Jose CA on-site relocation.`,
    comps: ['Computer Vision', 'YOLOv8', 'Real-Time Inference', 'OpenCV', 'Stable Diffusion / ControlNet', 'Multi-Provider AI Vision', 'Transformer Video Summarization', 'Multi-Modal LLM/VLM Integration', 'Python / PyTorch', 'San Jose CA On-Site'],
  },
  'arize-ai-engineer-instrumentation': {
    summary: `${BASE} Targeting Arize AI's AI Engineer (Instrumentation, OpenInference) seat — multi-provider AI vision SDK shipped to Chrome Web Store (Manga Lens, 4 providers w/ per-provider payload routing for failure isolation), eval-first RAG (~35% retrieval precision, >90% groundedness), agentic LLM workflows w/ tool discipline + grounding (>30% hallucination reduction), FastAPI + Docker packaging w/ structured logging (~30% post-deploy defect reduction), and Dream Decoder structured prompt transformation layers translate 1:1 to OpenInference + AI observability + multi-provider instrumentation work.`,
    comps: ['AI Observability', 'Instrumentation', 'OpenInference / OpenTelemetry', 'Multi-Provider AI Integration', 'LLM Tracing / Eval', 'Python / TypeScript', 'FastAPI / Docker', 'Structured Logging', 'Agentic LLM w/ Tool Discipline', 'Remote-US'],
  },
  'labelbox-fullstack-ai-data-platform': {
    summary: `${BASE} Targeting Labelbox's Full-Stack Engineer (AI Data Platform) seat — Suvidha video summarization HITL eval w/ ~85% alignment between AI-selected highlights and human-curated key moments, agentic claims pipeline w/ schema-validated JSON contracts + explainable risk scoring + audit-ready reasoning traces, eval-first RAG (~35% precision, >90% groundedness), Dream Decoder full-stack FastAPI + React/TypeScript multi-modal app w/ structured prompt transformation, and multi-provider AI vision Chrome extension (Manga Lens) translate 1:1 to Labelbox's HITL + RLHF + LLM-in-loop review + multi-modal eval tooling. Open to SF Bay Area Hybrid relocation.`,
    comps: ['Full-Stack AI Engineer', 'Human-in-the-Loop Eval', 'RLHF / Preference Data', 'LLM-in-Loop Review', 'AI Evaluation Frameworks', 'Multi-Modal (Text/Image/Video)', 'Schema-Validated Agent Contracts', 'Python / TypeScript / React / FastAPI', 'PostgreSQL + pgvector / Docker', 'SF Bay Area Hybrid'],
  },
};
