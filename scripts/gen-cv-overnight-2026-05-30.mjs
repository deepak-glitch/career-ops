#!/usr/bin/env node
// Tailors CV HTML for the 2026-05-30 overnight batch and generates PDFs.
// Usage: node scripts/gen-cv-overnight-2026-05-30.mjs <track> <slug> <summary-override> <competencies-csv>
//   track ∈ {us, intl}   (chooses output/ vs output-intl/)
import { readFileSync, writeFileSync, mkdirSync } from 'fs';
import { execSync } from 'child_process';
import yaml from 'js-yaml';

const [trackArg, slug, summaryOverride, competenciesCsv] = process.argv.slice(2);
const track = (trackArg || 'us').toLowerCase();
if (!slug || !summaryOverride) {
  console.error('usage: gen-cv-overnight-2026-05-30.mjs <us|intl> <slug> <summary> [comma,sep,competencies]');
  process.exit(1);
}
...
const DEFAULT_COMPETENCIES = [
  'Applied AI / LLMs', 'RAG + Grounding', 'Agentic Workflows', 'Schema Contracts',
  'Eval Harnesses', 'Python / FastAPI', 'TypeScript / React', 'Docker / CI-CD',
  'Multimodal Vision', 'Predictive ML',
];
const COMPETENCIES = competenciesCsv
  ? competenciesCsv.split(',').map(s => s.trim()).filter(Boolean)
...
const EXPERIENCE = `
    <div class="job">
      ...
      <ul>
        <li>Built <strong>Retrieval-Augmented Generation (RAG)</strong> for clinical knowledge retrieval; recursive semantic chunking + transformer embeddings improved retrieval precision <strong>~35%</strong>, reduced irrelevant retrieval <strong>&gt;30%</strong>, grounded-response alignment <strong>&gt;90%</strong>.</li>
...
const PROJECTS = `
    ...
    <div class="project">
      ...
      <div class="project-desc">Real-time manga translation extension shipped to the Chrome Web Store. Multi-provider abstraction (4 AI vision providers including Claude Sonnet) with per-provider payload handling for failure isolation. 29 sites supported across Manifest V3 service workers.</div>
      <div class="project-tech">TypeScript, Manifest V3, Service Workers, Claude / OpenAI / Google / Ollama</div>
    </div>
    <div class="project">
...
const SKILLS = `
    ...
    <span class="skill-item"><span class="skill-category">AI / ML:</span> PyTorch, scikit-learn, XGBoost, LangChain, LangGraph, LlamaIndex, OpenAI, Anthropic Claude, Hugging Face Transformers, Diffusers, YOLOv8</span>
...
const baseOut = track === 'intl' ? 'output-intl' : 'output';
const out = `${baseOut}/${DATE}/cv-deepak-mallampati-${slug}-${DATE}.pdf`;
writeFileSync(tmp, html, 'utf-8');
mkdirSync(`${baseOut}/${DATE}`, { recursive: true });
