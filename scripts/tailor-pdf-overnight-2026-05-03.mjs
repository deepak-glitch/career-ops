#!/usr/bin/env node
// Overnight 2026-05-03 batch 7 — ElevenLabs FDE-SWE + Scale AI Applied AI (Enterprise GenAI).
...
const ROLES = {
  'elevenlabs-fde-software-engineer': {
    summary: `${BASE} Targeting ElevenLabs's Forward Deployed Engineer (Software Engineer) seat — multi-provider AI vision Chrome Web Store extension (Manga Lens, 4 vision providers, viewport-slice DOM capture across 29 sites), agentic LLM workflows w/ structured outputs and grounding controls, multimodal video summarization (transformer-based, 5,000+ sessions, 60-70% review-time reduction), and production reliability (FastAPI + Docker + structured logging + load simulation) translate 1:1 to ElevenLabs voice/agents customer integrations.`,
    comps: ['Forward Deployed Engineer', 'Multi-Provider AI Integration', 'Agentic LLM Workflows', 'Production AI Reliability', 'Multimodal Video Understanding', 'Browser Automation (Manifest V3)', 'Python / FastAPI / TypeScript', 'Docker / Structured Logging', 'Customer-Facing Engineering', 'Cloud-Ready (AWS)'],
  },
  'scale-ai-applied-ai-engineer-enterprise-genai': {
    summary: `${BASE} Targeting Scale AI's Applied AI Engineer (Enterprise GenAI) seat — multi-modal AI agents w/ tool-calling (Agentic Healthcare Claims 5-stage pipeline w/ schema-validated JSON contracts + RAG-grounded CPT/ICD validation + ANN duplicate detection), production GenAI / LLM workflows (Progress Solutions w/ >30% hallucination reduction + ~25% agent stability gain), and Python + numpy/pandas EHR data pipelines (>98% dataset reliability) translate 1:1 to Scale AI's enterprise GenAI delivery model. Open to SF / NYC relocation.`,
    comps: ['Applied AI Engineer', 'Enterprise GenAI', 'Multi-Modal AI Agents w/ Tool-Calling', 'Production LLM Workflows', 'Schema-Validated JSON Contracts', 'RAG / Retrieval Grounding', 'Python / NumPy / Pandas', 'AWS / GCP Cloud Deployment', 'Evals / Structured Outputs', 'Hybrid SF / NYC'],
  },
};
...
if (targets.length === 0) {
  console.log('Usage: node tailor-pdf-overnight-2026-05-03.mjs <slug>...');
