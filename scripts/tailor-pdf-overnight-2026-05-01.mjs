#!/usr/bin/env node
// Overnight 2026-05-01 — Anthropic Beneficial Deployments / Boomi AI FDE / OneStream AI FDE.
...
const ROLES = {
  'anthropic-beneficial': {
    summary: `${BASE} Targeting Applied AI / LLM Engineer roles for mission-driven deployments — production LLM applications across ed-tech, healthcare, nonprofits, and scientific research; agent architectures with schema-validated contracts; evaluation frameworks with grounded-response gates; cost-aware multi-provider routing; and HIPAA-conscious governance for regulated domains.`,
    comps: ['Applied AI Engineering', 'LLM Application Development', 'Mission-Driven Deployments', 'Agent Architectures', 'Evaluation Frameworks', 'MCP / Agent Skills', 'Healthcare AI', 'Cost-Aware Multi-Provider Routing', 'Schema-Contract Agents', 'Public-Goods Tooling'],
  },
  'boomi-fde': {
    summary: `${BASE} Targeting AI Forward Deployed roles bridging enterprise integration and agentic AI — LangChain/CrewAI agent shipping, RAG with grounded outputs, REST/Docker packaging, and Order-to-Cash ERP delivery experience (T-SQL stored procedures, Jenkins CI/CD, Grafana observability) for compliance-sensitive customer environments.`,
    comps: ['AI Forward Deployed Engineering', 'Enterprise Integration', 'LangChain / CrewAI Agents', 'iPaaS Patterns', 'REST APIs / Docker', 'Order-to-Cash ERP Delivery', 'Jenkins CI/CD', 'Grafana Observability', 'Cloud-Native Deploy', 'Customer Translation'],
  },
  'onestream-fde': {
    summary: `${BASE} Targeting AI Forward Deployed Engineer roles on enterprise platforms — Python + SQL data foundations, ETL/ELT pipelines (EHR extracts, appointment histories, support ticket logs), REST API + Docker shipping, applied AI/ML in production (scikit-learn, XGBoost, LangChain), and compliance-sensitive financial-modules ERP delivery (Emerson Synthesis, RBAC, audit logging) bridging to FP&A / EPM platform work.`,
    comps: ['AI Forward Deployed Engineering', 'SQL + Data Modeling', 'ETL / ELT Pipelines', 'REST APIs / Docker', 'Python / Applied ML', 'Compliance-Sensitive ERP', 'Enterprise Platform Delivery', 'Solution Architecture', 'Customer Translation', 'Hypothesis-Driven Problem Solving'],
  },
};
...
const PROJECTS = `
    ...
    <div class="project">
      <span class="project-title">Patient Records Management Web Application</span>
      <div class="project-desc">Bottle + SQLite relational healthcare CRUD app with normalized patients/diagnoses schema, foreign-key constraints, application-layer cascade deletes, and LEFT JOIN + GROUP_CONCAT aggregation for single-query patient+diagnosis retrieval.</div>
      <div class="project-tech">Python, Bottle, SQLite, normalized schema</div>
    </div>`;
...
if (targets.length === 0) {
  console.log('Usage: node tailor-pdf-overnight-2026-05-01.mjs <slug>...');
