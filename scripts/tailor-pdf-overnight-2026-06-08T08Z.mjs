#!/usr/bin/env node
/**
 * Overnight 2026-06-08T08Z tailor: 3 PDFs (Coval US, supermodular Intl, Arva AI Intl).
 * Mirrors scripts/tailor-pdfs-batch.mjs structure.
 */
...
const JOBS = [
  {
    slug: 'coval-forward-deployed-engineer-2026-06-08',
    track: 'us',
    summary: "Applied AI engineer with ~3.5 yrs shipping production agentic LLM systems + evaluation harnesses in regulated healthcare. Built RAG-grounded pipelines (~35% retrieval precision, >90% grounded alignment) and agentic conductor-subagent workflows (42% token reduction, >30% hallucination cut). Ready to embed with customer voice/chat AI teams, build the bridges that make Coval work in production, and translate real customer friction into product features.",
    competencies: [
      'Forward Deployed', 'LLM Evaluation', 'Agentic Workflows', 'Voice/Chat Agents',
      'RAG + Grounding', 'Python / FastAPI', 'Customer Discovery', 'Production AI', 'Fast Prototyping', 'Docker',
    ],
  },
  {
    slug: 'supermodular-forward-deployed-security-engineer-2026-06-08',
    track: 'intl',
    summary: "Applied AI engineer with ~3.5 yrs hardening production AI in regulated healthcare and enterprise environments. Designed guardrails, schema-validated agent contracts, and evaluation loops that cut hallucinations >30% and post-deploy defects ~30%. Comfortable working inside complex, imperfect system landscapes - building safeguards across auth, data access, and tool boundaries so AI systems run safely at enterprise scale.",
    competencies: [
      'Forward Deployed', 'AI Security', 'Guardrails', 'Prompt Injection Mitigation',
      'Evaluation Loops', 'Reliability', 'Production AI', 'Python / FastAPI', 'Observability', 'Docker',
    ],
  },
  {
    slug: 'arva-ai-forward-deployed-engineer-2026-06-08',
    track: 'intl',
    summary: "Applied AI engineer with ~3.5 yrs shipping production agentic LLM systems in regulated banking and healthcare. Built multi-agent review pipelines with schema-validated contracts (cut hallucinations >30%), RAG-grounded compliance retrieval (~35% precision gain), and customer-embedded delivery from Vanguard banking enterprise rollout. Ready to lead Arva pilot deployments end-to-end - frame the problem with banks, ship rapidly, validate at go-live.",
    competencies: [
      'Forward Deployed', 'Customer Pilot Delivery', 'AML/KYC Domain Adjacent', 'Multi-Agent Systems',
      'Regulated Banking', 'RAG + Grounding', 'Python / FastAPI', 'Fast Prototyping', 'Schema Contracts', 'Docker',
    ],
  },
];
...
const dateFolder = '2026-06-08';
execSync(`mkdir -p output/${dateFolder} output-intl/${dateFolder}`);

let done = 0, failed = [];
for (const job of JOBS) {
  const html = buildHtml(job);
  const tmpPath = `/tmp/cv-${job.slug}.html`;
  const outDir = job.track === 'intl' ? 'output-intl' : 'output';
  const pdfPath = `${outDir}/${dateFolder}/cv-deepak-mallampati-${job.slug}.pdf`;
