#!/usr/bin/env node
// Usage: node scripts/gen-cv-overnight.mjs <slug> <date> <track:us|intl> <summary>
import { readFileSync, writeFileSync, mkdirSync } from 'fs';
import { execSync } from 'child_process';
import yaml from 'js-yaml';

const [slug, date, track, summaryOverride] = process.argv.slice(2);
if (!slug || !date || !track) { console.error('usage: gen-cv-overnight.mjs <slug> <date> <track> [summary]'); process.exit(1); }
...
const outDir = track === 'intl' ? `output-intl/${date}` : `output/${date}`;
