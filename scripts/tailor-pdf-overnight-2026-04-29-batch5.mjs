#!/usr/bin/env node
// Per-role tailoring helper for overnight 2026-04-29 fifth batch (#287 Canopy Connect, #292 Innovaccer).
...
const ROLES = {
  'canopy-connect-fde': {
    summary: `${BASE} Targeting Forward Deployed Engineer roles for API-first insurance and data platforms — multi-provider integration depth, FastAPI + REST shipping, customer-facing stakeholder docs, and full-stack TypeScript/React from Manga Lens; comfortable owning end-to-end customer onboarding and API troubleshooting.`,
    comps: ['Forward Deployed Engineering', 'API Onboarding + Integration', 'Customer-Embedded Delivery', 'Full-Stack TypeScript / React', 'FastAPI / Docker', 'PostgreSQL / Data Pipelines', 'Multi-Provider Integration', 'AWS-Ready Deployment', 'Stakeholder Communication', 'Enterprise Authentication / Auth Flows'],
  },
  'innovaccer-fde-ai': {
    summary: `${BASE} Targeting AI Forward Deployed Engineer roles in healthcare AI vendors — direct domain fit (HIPAA-conscious shipping, healthcare RAG, agentic claims pipeline) and direct archetype fit (production LLM apps + agents with tool calling + REST APIs over Python/FastAPI/Docker).`,
    comps: ['Forward Deployed Engineering', 'Healthcare AI (HIPAA-Conscious)', 'Agentic LLM Workflows', 'Production RAG', 'Vector Search + Embeddings', 'REST APIs / FastAPI', 'Python / Docker', 'AWS-Ready Microservices', 'Agent Tool Calling', 'Customer-Embedded Delivery'],
  },
};
...
if (targets.length === 0) {
  console.log('Usage: node tailor-pdf-overnight-2026-04-29-batch5.mjs <slug>...');
