#!/usr/bin/env node
// Overnight 2026-05-07 batch 2 — Pulsora (#613) Applied AI Engineer.
// Only #613 from this batch scored >= 3.0; others below threshold.
...
const ROLES = {
  'pulsora-applied-ai-engineer': {
    summary: `${BASE} Targeting Pulsora's Applied AI Engineer (Remote-US) seat — Manga Lens 4-provider AI vision Chrome MV3 extension live on Chrome Web Store w/ cost-aware payload routing (direct multi-provider production analog to Pulsora's OpenAI/Anthropic/Gemini/open-source ask), Agentic Healthcare Claims 5-stage agent pipeline w/ schema-validated JSON contracts between agents (multi-step reasoning + memory + tool use 1:1), Dream Decoder full-stack FastAPI + React/TypeScript/Vite/Tailwind app w/ structured prompt transformation layers (~30% better contextual alignment, ~25-30% first-pass image success — full lifecycle backend + frontend shipping), production RAG w/ recursive chunking + grounding rules (~35% retrieval precision, >30% hallucination reduction), and FastAPI/Docker packaging w/ structured logging + load simulation (~30% post-deploy defects) translate 1:1 to Pulsora's enterprise ESG sustainability platform. AI-first workflows (Cursor/Claude Code/Copilot in personal projects). Open to US-Remote; F-1 OPT (sponsorship welcome).`,
    comps: ['Multi-Provider LLM Integration', 'Agent Workflows (RAG + Memory + Tool Use)', 'Schema-Validated JSON Agent Contracts', 'Eval Frameworks for AI Outputs', 'Full-Stack Shipping (FastAPI + React/TypeScript)', 'AI-First Development (Cursor/Claude Code/Copilot)', 'Production Reliability + Cost/Latency Optimization', 'Audit Trails + Compliance-Adjacent Discipline', 'Python Backend / FastAPI / Docker', 'Open to US-Remote; F-1 OPT'],
  },
};
...
const SKILLS = `
    ...
    <span class="skill-item"><span class="skill-category">Domains:</span> Applied AI, Healthcare AI, Compliance-adjacent (HIPAA → ESG transferable), Agentic / Multi-Step LLM, Production RAG, Multimodal Systems</span>`;
...
if (targets.length === 0) {
  console.log('Usage: node tailor-pdf-overnight-2026-05-07-batch2.mjs <slug>...');
