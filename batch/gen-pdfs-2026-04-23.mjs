#!/usr/bin/env node
// One-shot PDF generator for 2026-04-23 overnight batch.
// Reuses the regen-all.mjs pattern — shared base CV content, archetype-tagged
// summary per role. Reports are already written with full A-H analysis;
// this just produces the ATS PDF artifacts in output/2026-04-23/.
...
const BATCH = [
  ['mistral-ai-csi-emea-2026-04-23',               'reports/111-mistral-ai-csi-emea-2026-04-23.md'],
  ['salesforce-ai-fde-early-career-2026-04-23',    'reports/112-salesforce-ai-fde-early-career-2026-04-23.md'],
  ['databricks-ai-fde-2026-04-23',                 'reports/113-databricks-ai-fde-2026-04-23.md'],
  ['llamaindex-multimodal-ai-engineer-2026-04-23', 'reports/114-llamaindex-multimodal-ai-engineer-2026-04-23.md'],
  ['furtherai-fde-new-grad-2026-04-23',            'reports/115-furtherai-fde-new-grad-2026-04-23.md'],
  ['retell-ai-founding-fde-2026-04-23',            'reports/116-retell-ai-founding-fde-2026-04-23.md'],
  ['synth-founding-fde-ai-product-2026-04-23',     'reports/117-synth-founding-fde-ai-product-2026-04-23.md'],
  ['wisdomai-nlp-ml-engineer-2026-04-23',          'reports/118-wisdomai-nlp-ml-engineer-2026-04-23.md'],
];
...
console.log(`Generating ${BATCH.length} PDFs for 2026-04-23 batch...\n`);

execSync(`mkdir -p output/2026-04-23`);

let done = 0, failed = [];
for (const [slug, reportPath] of BATCH) {
  const html = buildHtml(slug, reportPath);
  const tmpPath = `/tmp/cv-${slug}.html`;
  const pdfPath = `output/2026-04-23/cv-deepak-mallampati-${slug}.pdf`;
  writeFileSync(tmpPath, html, 'utf-8');
  try {
    execSync(`node generate-pdf.mjs "${tmpPath}" "${pdfPath}"`, { stdio: ['ignore', 'ignore', 'ignore'] });
    done++;
    process.stdout.write(`  [${done}/${BATCH.length}] ${slug}\n`);
  } catch (err) {
...
console.log(`\nDone. ${done}/${BATCH.length} generated, ${failed.length} failed.`);
