#!/usr/bin/env node
// Overnight batch PDF generator for 2026-04-28 — pipeline reports >= 3.0 (only #223 Unison this batch).
...
const ROLES = [
  {
    slug: 'unison-ai-engineer-2026-04-28',
    company: 'Unison',
    role: 'AI Engineer (U.S. Remote)',
    summary:
      'Applied AI engineer (Master of Science, Computer Science) building production LLM-powered applications with RAG and agentic workflows. Healthcare RAG with recursive semantic chunking and transformer embeddings improved retrieval precision ~35% with >90% grounded alignment. Agentic LLM workflows for multi-step healthcare queries — tool discipline, structured outputs, and grounding rules — raised response stability ~25% and cut hallucinations >30%. Multi-agent claims pipeline uses schema-validated JSON contracts between agents to prevent cascading hallucinations, with RAG-grounded CPT/ICD validation against vector-indexed policies. FastAPI / Docker production services with structured logging cut post-deploy defects ~30%. Python primary; TypeScript on shipped projects; willing to ramp C# / .NET in onboarding.',
    competencies: [
