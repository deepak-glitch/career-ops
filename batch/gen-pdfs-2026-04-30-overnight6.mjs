#!/usr/bin/env node
// Overnight 2026-04-30 batch 6 PDF generator - LaunchDarkly (#327), Cresta (#329), NVIDIA (#331).
...
const ROLES = [
  {
    slug: 'launchdarkly-sdk-ai-2026-04-30',
    company: 'LaunchDarkly',
    role: 'SDK Software Engineer, AI',
    summary:
      'Applied AI engineer (M.S. CS, Kent State, May 2025; 2.5+ yrs) shipping production AI SDK-shaped artifacts in Python and TypeScript. Manga Lens (Chrome Web Store) is a multi-provider AI vision SDK with 4-provider abstraction (Claude / GPT-4o mini / GPT-4.1 Nano / Ollama), per-provider payload routing (WebP cloud, JPEG Ollama after CUDA crashes), 7-day translation cache, and Manifest V3 service workers across 29 site selectors - the same SDK-shape as the JD. Dream Decoder structured-prompt transformation layer lifted contextual alignment ~30% and first-pass image success ~25-30% over naive direct-prompt orchestration - DX-grade ergonomic gains. Production Python/FastAPI inference services with Docker, structured logging, and load simulation cut post-deploy defects ~30% at Progress Solutions. PyTorch + Hugging Face Transformers + Diffusers across LLM and multimodal projects (Pixel agent orchestrator, Manga Lens). Strong unit / integration / load-test culture; bias toward shipping public artifacts over decks.',
    competencies: [
      'AI SDK Engineering',
      'Python + TypeScript',
  ...
  {
    slug: 'cresta-fde-aiagent-2026-04-30',
    company: 'Cresta',
    role: 'Senior Forward Deployed Engineer (AI Agent)',
    summary:
      'Applied AI engineer (M.S. CS, Kent State, May 2025; 2.5+ yrs) building agentic LLM systems - the exact shape Cresta needs. Five-agent claims-processing pipeline (Intake / Validation / Consistency / Duplicate / Risk Scoring) uses schema-validated JSON contracts (pydantic) between agents to prevent cascading hallucinations, with RAG-grounded CPT/ICD validation, ANN duplicate detection, and audit-ready reasoning traces. Healthcare RAG (recursive semantic chunking + transformer embeddings) lifted retrieval precision ~35% with >90% grounded alignment; agentic LLM workflows for multi-step healthcare queries (eligibility checks, care navigation, documentation clarification) cut hallucinations >30% and lifted response stability ~25% via structured reasoning, tool discipline, and grounding rules. Pixel agent orchestrator decomposes high-level prompts into bounded generation tasks - LLM-as-orchestrator pattern. FastAPI / Docker production services with structured logging cut post-deploy defects ~30%. Customer-facing instinct - stakeholder system-limitation docs, Manga Lens public ship to Chrome Web Store, comfort owning end-to-end delivery.',
    competencies: [
  ...
  {
    slug: 'nvidia-silicon-codesign-newgrad-2026-04-30',
    company: 'NVIDIA',
    role: 'Applied AI Engineer, Silicon Co-Design Group, New College Grad 2026',
    summary:
      'Applied AI engineer (M.S. Computer Science, Kent State University, May 2025) bringing shipped agentic + RAG + MLOps systems to silicon co-design. Five-agent claims pipeline with schema-validated JSON contracts (pydantic) between agents - directly transferable to "AI assistants and agentic systems for SCG engineers using RAG, tool-calling, and fine-tuned models." Healthcare RAG lifted retrieval precision ~35% with >90% grounded alignment; agentic LLM workflows for multi-step queries cut hallucinations >30% and lifted response stability ~25% via structured reasoning, tool discipline, and grounding rules. Predictive ML (scikit-learn, XGBoost) for high-risk patient cohorts lifted recall +15-20% with precision >90%. PyTorch and Hugging Face Transformers across LLM, vision (YOLOv8), and multimodal (Pixel agent + Diffusers) projects. CNN (YOLOv8 unified detect+classify, ~30% latency reduction) and graph-domain (NetworkX road-graph community detection on 20k-node subgraph) hands-on. FastAPI / Docker production services with structured logging cut post-deploy defects ~30% - the data + MLOps shape NVIDIA needs to scale silicon co-design tooling.',
    competencies: [
