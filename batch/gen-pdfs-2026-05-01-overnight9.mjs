#!/usr/bin/env node
// Overnight 2026-05-01 batch 9 PDF generator - Cooper AI/Nirvana (#379), Fingerpaint (#381).
...
const ROLES = [
  {
    slug: 'cooper-ai-founding-fde-2026-05-01',
    company: 'Cooper AI (Nirvana)',
    role: 'Founding Forward Deployed Engineer',
    summary:
      'Applied AI engineer (M.S. Computer Science, Kent State, May 2025; 2.5+ yrs production) shipping deploy-and-integrate AI workflows across regulated domains - directly transferable to commercial-insurance ops at Cooper AI. At Progress Solutions: healthcare RAG (recursive semantic chunking + transformer embeddings, +35% retrieval precision, >90% grounded alignment, >30% irrelevant retrieval cut) and agentic LLM workflows for multi-step queries (eligibility, care navigation, documentation clarification) - hallucinations >30% cut, agent stability +25%. Five-agent claims pipeline with schema-validated JSON contracts (pydantic) between Intake / Validation / Consistency / Duplicate / Risk Scoring agents prevents cascading hallucinations and produces audit-ready reasoning traces - the deploy-into-real-workflow + orchestration + evals stack the JD names. Manga Lens shipped solo to the Chrome Web Store: 4 AI vision providers (Claude / GPT-4o mini / GPT-4.1 Nano / Ollama) with per-provider payload routing (WebP cloud, JPEG Ollama after CUDA crashes), Manifest V3 service workers, 7-day cache, 29 site selectors - integration discipline with messy 3rd-party APIs. Dream Decoder full-stack FastAPI + React / Vite multimodal: structured-prompt transformation lifted contextual alignment ~30%, first-pass image success ~25-30%. FastAPI + Docker production with structured logging cut post-deploy defects ~30% in HIPAA-conscious environments.',
    competencies: [
  ...
  {
    slug: 'fingerpaint-ai-agent-engineer-2026-05-01',
    company: 'Fingerpaint Group',
    role: 'AI Agent Engineer',
    summary:
      'Applied AI engineer (M.S. Computer Science, Kent State, May 2025; 2.5+ yrs production) architecting and shipping AI agent solutions on top of frontier LLMs - direct fit for Fingerpaint\'s AI Agent Engineer mandate at the pharma / healthcare advertising intersection. Five-agent healthcare claims pipeline with schema-validated JSON contracts (pydantic) between Intake / Validation / Consistency / Duplicate / Risk Scoring agents prevents cascading hallucinations and produces audit-ready reasoning traces - same architectural pattern Fingerpaint needs for client-side AI workflows. Manga Lens shipped solo to the Chrome Web Store: full-stack browser-native AI product on Manifest V3 service workers + content scripts, 4 vision LLM providers (Claude / GPT-4o mini / GPT-4.1 Nano / Ollama) with per-provider payload routing (WebP cloud, JPEG Ollama after CUDA crashes), 7-day cache, 29 site selectors - "prototype to production" + integration discipline. Dream Decoder full-stack FastAPI + React / Vite + Tailwind multimodal app: intermediate structured-prompt transformation lifted contextual alignment ~30%, first-pass image success ~25-30%. Healthcare RAG +35% retrieval precision, >90% grounded alignment under HIPAA-conscious governance - same regulatory floor (MLR review, drug-mention compliance) pharma marketing requires. Python + TypeScript primary, FastAPI + Docker production discipline, Cursor / frontier LLM toolchain in daily use.',
    competencies: [
      'AI Agent Architecture',
      'Frontier LLMs (Claude / GPT-4o / Gemini)',
      'Schema-Validated Contracts',
      'Full-Stack React + FastAPI',
      'Python + TypeScript',
      'API Integration + Multi-Provider',
      'Docker / Cloud-Ready',
      'Responsible AI / Audit Trails',
      'Healthcare + Pharma Adjacency',
      'Rapid Prototype to Production',
    ],
  },
];
