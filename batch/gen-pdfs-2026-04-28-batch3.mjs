#!/usr/bin/env node
// Overnight batch 3 PDF generator for 2026-04-28 — LangChain Deployed Engineer (Phoenix, #234).
...
const ROLES = [
  {
    slug: 'langchain-deployed-engineer-2026-04-28',
    company: 'LangChain',
    role: 'Deployed Engineer (Phoenix)',
    summary:
      'Applied AI engineer (Master of Science, Computer Science, 2.5+ yrs) shipping production agentic LLM systems on the LangChain stack. Healthcare RAG with recursive semantic chunking and transformer embeddings improved retrieval precision ~35% with >90% grounded alignment, validated through clinical SME-in-the-loop evals. Multi-agent claims pipeline (Intake → Validation → Consistency → Duplicate → Risk Scoring) uses schema-validated JSON contracts (pydantic) between agents to prevent cascading hallucinations, with audit-ready reasoning traces — agent-based work that goes beyond simple API calls. Customer-facing track via clinical SME collaboration on RAG eval and Manga Lens — a public Chrome Web Store ship across 29 sites with active users. Multi-language production stack: Python (healthcare RAG, agentic claims pipeline) and JavaScript / TypeScript (Manga Lens browser extension, Pixel agent orchestrator). Comfortable co-architecting with customer engineering teams, advising on architecture and best practices, and supporting POCs / architecture reviews / technical evaluations end-to-end.',
    competencies: [
...
const EXPERIENCE = `
    <div class="job">
      ...
      <ul>
        <li>Built <strong>Retrieval-Augmented Generation (RAG)</strong> on the LangChain stack for clinical knowledge retrieval; recursive semantic chunking and transformer embeddings improved retrieval precision <strong>~35%</strong>, reduced irrelevant retrieval <strong>&gt;30%</strong>, retrieval-grounded response alignment <strong>&gt;90%</strong>.</li>
        ...
        <li>Designed <strong>evaluation pipelines</strong> for LLM output quality and hallucination mitigation; SME-in-the-loop side-by-side eval, audit trails, and stakeholder-facing system-limitation docs.</li>
        <li>Co-architected with clinical SMEs on RAG groundedness criteria and agent failure modes — customer-facing technical communication on tradeoffs (chunking strategies, retrieval thresholds, structured-output schemas).</li>
        ...
        <li>EHR preprocessing (Pandas, NumPy); dataset reliability <strong>&gt;98%</strong>, downstream instability <strong>-40%</strong>. HIPAA-conscious governance.</li>
      </ul>
    </div>
    <div class="job">
      <div class="job-header">
...
const PROJECTS = `
    <div class="project">
      ...
      <div class="project-desc">5-stage multi-agent pipeline (Intake, Validation, Consistency, Duplicate, Risk Scoring) with schema-validated JSON contracts between agents to prevent cascading hallucinations. RAG-grounded CPT/ICD validation against vector-indexed policy documents. Audit-ready explainable risk scoring with reasoning traces — agent-based work beyond simple API calls.</div>
    ...
    <div class="project">
      ...
      <div class="project-desc">End-to-end multimodal app: text description, intermediate structured-prompt transformation, grounded image generation. Structured-prompt layer lifted contextual alignment ~30% and first-pass image success ~25-30% over naive direct-prompt orchestration — architecture-review-grade tradeoff documentation.</div>
...
const SKILLS = `
    ...
    <span class="skill-item"><span class="skill-category">AI / ML:</span> PyTorch, scikit-learn, XGBoost, LangChain, LangGraph, LlamaIndex, Hugging Face Transformers, Diffusers, YOLOv8</span>
    <span class="skill-item"><span class="skill-category">LLM / GenAI:</span> RAG, agentic workflows, multi-agent orchestration, structured outputs, prompt engineering, evaluation pipelines, guardrails, grounding, embeddings, vector search</span>
    ...
    <span class="skill-item"><span class="skill-category">Domains:</span> Healthcare AI (HIPAA-conscious), Applied AI, Customer Engineering, Forward Deployed, Multimodal</span>`;
