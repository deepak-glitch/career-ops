#!/usr/bin/env node
// One-shot helper to generate PDFs for overnight batch reports 450, 453, 454.
// Reuses the canonical template logic from regen-all.mjs.
...
const BASE_SUMMARY = `Applied AI engineer (2.5+ years) shipping production LLM systems end to end: healthcare RAG (~35% retrieval precision, >90% grounded alignment), agentic LLM workflows (>30% hallucination reduction), schema-validated multi-agent pipelines, and FastAPI + Docker packaging (~30% post-deploy defect reduction). Solo-shipped Manga Lens to the Chrome Web Store with four AI vision providers under per-provider failure isolation. Master's in Computer Science, Kent State (May 2025). Ready to ship production AI systems from day one. US-based, F-1 OPT, open to sponsorship.`;
...
function buildHtml(roleSummaryTag) {
  const taggedSummary = `${BASE_SUMMARY} ${roleSummaryTag}`;
...
const TARGETS = [
  {
    slug: 'scale-ai-software-engineer-new-grad',
    summaryTag: 'Targeting AI Solutions / Internal AI Platform engineering roles building customer-facing RAG, fraud-detection, and ML-quality tooling at scale (Scale AI New Grad and similar).',
  },
  {
    slug: 'avent-founding-fde',
    summaryTag: 'Targeting Founding Forward Deployed Engineer roles at YC startups deploying AI agents inside real customer workflows (Avent and similar industrial-commerce + B2B distribution AI startups). Founder DNA from E-Farming and Manga Lens.',
  },
  {
    slug: 'hatch-forward-deployed-engineer',
    summaryTag: 'Targeting Forward Deployed Engineer roles deploying LLM and generative-AI customer experiences at AI CSR / voice / SMS / email startups (Hatch and similar).',
  },
];
...
for (const t of TARGETS) {
  const html = buildHtml(t.summaryTag);
  const tmpPath = `/tmp/cv-${t.slug}-${TODAY}.html`;
  const pdfPath = `output/${TODAY}/cv-deepak-mallampati-${t.slug}-${TODAY}.pdf`;
  writeFileSync(tmpPath, html, 'utf-8');
  try {
    execSync(`node generate-pdf.mjs "${tmpPath}" "${pdfPath}"`, { stdio: ['ignore', 'pipe', 'pipe'] });
    done++;
    console.log(`[${done}/${TARGETS.length}] ${t.slug} → ${pdfPath}`);
  } catch (err) {
    failed.push({ slug: t.slug, err: err.message?.slice(0, 200) });
    console.log(`[FAIL] ${t.slug}: ${err.message?.slice(0, 200)}`);
  }
}

console.log(`\nDone. ${done}/${TARGETS.length} generated, ${failed.length} failed.`);
